...
function renderGrid(list) {
  ...
  grid.innerHTML = filtered.map(a => {
    ...
    return `
      <div class="athlete-card p-8 bg-[#121212] rounded-[40px] border border-white/5 text-center shadow-2xl">
        <div class="text-[2.2rem] font-black italic uppercase tracking-tight leading-none text-white">${a.name}</div>
        <div class="mt-4 text-slate-500 font-bold tracking-[0.2em] uppercase text-[11px] flex items-center justify-center">
          <span class="inline-block w-1.5 h-1.5 rounded-full bg-emerald-500 mr-2.5"></span>
          ${a.sport} • ${a.team || "Individual"}
        </div>
        
        <a href="${shopUrl}" target="_blank" rel="noopener noreferrer"
           class="shop-btn mt-8 flex flex-col items-center justify-center w-full py-4 rounded-full bg-[#f2f20d] text-black hover:scale-[1.02] active:scale-[0.98] transition-all duration-200">
          <span class="font-black uppercase tracking-wider text-[14px]">SHOP COLLECTIBLES</span>
          ${money ? `<span class="mt-0.5 text-[10px] font-bold opacity-80">(AVG LIST: ${money})</span>` : ``}
        </a>
      </div>
    `;
  }).join("");
